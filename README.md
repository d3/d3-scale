...
For continuous quantitative data, you typically want a [linear scale](#linear-scales), though sometimes it’s useful to transform the data using a [power scale](#power-scales) or [log scale](#log-scales). To aid differentiation, consider a [quantize scale](#quantize-scale), which rounds continuous data to a fixed set of discrete values; similarly, a [quantile scale](#quantile-scale) computes quantiles from a sample population, and a [threshold scale](#threshold-scale) allows you to specify arbitrary thresholds to specify discrete breaks in continuous data.

For ordinal or categorical data, use an [ordinal scale](#ordinal-scale). Several built-in [categorical color scales](#category-scales) are provided.

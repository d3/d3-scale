...
it("quantize(value) maps a number to a discrete value in the range", () => {
  const s = d3.scaleQuantize().range([0, 1, 2]);
...
it("quantize(value) clamps input values to the domain", () => {
  const a = {},
      b = {},
      c = {},
      s = d3.scaleQuantize().range([a, b, c]);
...
it("quantize.unknown(value) sets the return value for undefined, null, and NaN input", () => {
  const s = d3.scaleQuantize().range([0, 1, 2]).unknown(-1);
...
it("quantize.domain() coerces domain values to numbers", () => {
  const s = d3.scaleQuantize().domain(["-1.20", "2.40"]);
...
it("quantize.domain() accepts an iterable", () => {
  const s = d3.scaleQuantize().domain(new Set([1, 2]));
  assert.deepStrictEqual(s.domain(), [1, 2]);
});

it("quantize.domain() only considers the first and second element of the domain", () => {
  const s = d3.scaleQuantize().domain([-1, 100, 200]);
  assert.deepStrictEqual(s.domain(), [-1, 100]);
});

it("quantize.range() cardinality determines the degree of quantization", () => {
  const s = d3.scaleQuantize();
  assertInDelta(s.range(array.range(0, 1.001, 0.001))(1/3), 0.333, 1e-6);
  assertInDelta(s.range(array.range(0, 1.010, 0.010))(1/3), 0.330, 1e-6);
  assertInDelta(s.range(array.range(0, 1.100, 0.100))(1/3), 0.300, 1e-6);
  assertInDelta(s.range(array.range(0, 1.200, 0.200))(1/3), 0.400, 1e-6);
  assertInDelta(s.range(array.range(0, 1.250, 0.250))(1/3), 0.250, 1e-6);
  assertInDelta(s.range(array.range(0, 1.500, 0.500))(1/3), 0.500, 1e-6);
  assertInDelta(s.range(array.range(1))(1/3), 0, 1e-6);
});
...
it("quantize.invertExtent() maps a value in the range to a domain extent", () => {
  const s = d3.scaleQuantize().range([0, 1, 2, 3]);
...
it("quantize.invertExtent() returns the first match if duplicate values exist in the range", () => {
  const s = d3.scaleQuantize().range([0, 1, 2, 0]);
  assert.deepStrictEqual(s.invertExtent(0), [0.00, 0.25]);
  assert.deepStrictEqual(s.invertExtent(1), [0.25, 0.50]);
});

it("quantize.invertExtent(y) is exactly consistent with quantize(x)", () => {
  const s = d3.scaleQuantize().domain([4.2, 6.2]).range(array.range(10));

...
it("symlog(x) maps a domain value x to a range value y", () => {
  const s = d3.scaleSymlog().domain([-100, 100]);
...
it("symlog.invert(y) coerces range values to numbers", () => {
  const s = d3.scaleSymlog().range(["-3", "3"]);
  assert.deepStrictEqual(s.invert(3), 1);
});

it("symlog.invert(y) returns NaN if the range is not coercible to number", () => {
  assert(isNaN(d3.scaleSymlog().range(["#000", "#fff"]).invert("#999")));
  assert(isNaN(d3.scaleSymlog().range([0, "#fff"]).invert("#999")));
});

it("symlog.constant(constant) sets the constant to the specified value", () => {
  const s = d3.scaleSymlog().constant(5);
  assert.strictEqual(s.constant(), 5);
});

it("symlog.constant(constant) changing the constant does not change the domain or range", () => {
  const s = d3.scaleSymlog().constant(2);
  assert.deepStrictEqual(s.domain(), [0, 1]);
  assert.deepStrictEqual(s.range(), [0, 1]);
});

it("symlog.domain(domain) accepts an array of numbers", () => {
  assert.deepStrictEqual(d3.scaleSymlog().domain([]).domain(), []);
  assert.deepStrictEqual(d3.scaleSymlog().domain([1, 0]).domain(), [1, 0]);
  assert.deepStrictEqual(d3.scaleSymlog().domain([1, 2, 3]).domain(), [1, 2, 3]);
});
...
it("symlog.domain(domain) makes a copy of domain values", () => {
  const d = [1, 2], s = d3.scaleSymlog().domain(d);
...
it("symlog.domain() returns a copy of domain values", () => {
  const s = d3.scaleSymlog(), d = s.domain();
...
it("symlog.range(range) does not coerce range to numbers", () => {
  const s = d3.scaleSymlog().range(["0px", "2px"]);
...
it("symlog.range(range) can accept range values as arrays or objects", () => {
  assert.deepStrictEqual(d3.scaleSymlog().range([{color: "red"}, {color: "blue"}])(1), {color: "rgb(0, 0, 255)"});
  assert.deepStrictEqual(d3.scaleSymlog().range([["red"], ["blue"]])(0), ["rgb(255, 0, 0)"]);
});

it("symlog.range(range) makes a copy of range values", () => {
  const r = [1, 2], s = d3.scaleSymlog().range(r);
...
it("symlog.range() returns a copy of range values", () => {
  const s = d3.scaleSymlog(), r = s.range();
...
it("symlog.clamp() is false by default", () => {
  assert.strictEqual(d3.scaleSymlog().clamp(), false);
  assert.strictEqual(d3.scaleSymlog().range([10, 20])(3), 30);
  assert.strictEqual(d3.scaleSymlog().range([10, 20])(-1), 0);
  assert.strictEqual(d3.scaleSymlog().range([10, 20]).invert(30), 3);
  assert.strictEqual(d3.scaleSymlog().range([10, 20]).invert(0), -1);
});

it("symlog.clamp(true) restricts output values to the range", () => {
  assert.strictEqual(d3.scaleSymlog().clamp(true).range([10, 20])(2), 20);
  assert.strictEqual(d3.scaleSymlog().clamp(true).range([10, 20])(-1), 10);
});

it("symlog.clamp(true) restricts input values to the domain", () => {
  assert.strictEqual(d3.scaleSymlog().clamp(true).range([10, 20]).invert(30), 1);
  assert.strictEqual(d3.scaleSymlog().clamp(true).range([10, 20]).invert(0), 0);
});

it("symlog.clamp(clamp) coerces the specified clamp value to a boolean", () => {
  assert.strictEqual(d3.scaleSymlog().clamp("true").clamp(), true);
  assert.strictEqual(d3.scaleSymlog().clamp(1).clamp(), true);
  assert.strictEqual(d3.scaleSymlog().clamp("").clamp(), false);
  assert.strictEqual(d3.scaleSymlog().clamp(0).clamp(), false);
});

it("symlog.copy() returns a copy with changes to the domain are isolated", () => {
  const x = d3.scaleSymlog(), y = x.copy();
  x.domain([1, 2]);
...
it("symlog.copy() returns a copy with changes to clamping are isolated", () => {
  const x = d3.scaleSymlog().clamp(true), y = x.copy();
  x.clamp(false);
...
it("symlog().clamp(true).invert(x) cannot return a value outside the domain", () => {
  const x = d3.scaleSymlog().domain([1, 20]).clamp(true);

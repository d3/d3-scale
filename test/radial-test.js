...
it("scaleRadial(range) sets the range", () => {
  const s = d3.scaleRadial([100, 200]);
  assert.deepStrictEqual(s.domain(), [0, 1]);
...
it("scaleRadial(domain, range) sets the range", () => {
  const s = d3.scaleRadial([1, 2], [10, 20]);
...
it("radial(x) maps a domain value x to a range value y", () => {
  assert.strictEqual(d3.scaleRadial([1, 2])(0.5), 1.5811388300841898);
});

it("radial(x) ignores extra range values if the domain is smaller than the range", () => {
  assert.strictEqual(d3.scaleRadial().domain([-10, 0]).range([2, 3, 4]).clamp(true)(-5), 2.5495097567963922);
  assert.strictEqual(d3.scaleRadial().domain([-10, 0]).range([2, 3, 4]).clamp(true)(50), 3);
});

it("radial(x) ignores extra domain values if the range is smaller than the domain", () => {
  assert.strictEqual(d3.scaleRadial().domain([-10, 0, 100]).range([2, 3]).clamp(true)(-5), 2.5495097567963922);
  assert.strictEqual(d3.scaleRadial().domain([-10, 0, 100]).range([2, 3]).clamp(true)(50), 3);
});

it("radial(x) maps an empty domain to the middle of the range", () => {
  assert.strictEqual(d3.scaleRadial().domain([0, 0]).range([1, 2])(0), 1.5811388300841898);
  assert.strictEqual(d3.scaleRadial().domain([0, 0]).range([2, 1])(1), 1.5811388300841898);
});

it("radial(x) can map a bilinear domain with two values to the corresponding range", () => {
  const s = d3.scaleRadial().domain([1, 2]);
  assert.deepStrictEqual(s.domain(), [1, 2]);
...
it("radial.unknown(unknown)(NaN) returns the specified unknown value", () => {
  assert.strictEqual(d3.scaleRadial().unknown("foo")(NaN), "foo");
});
...
it("radial(x) can clamp negative values", () => {
  assert.strictEqual(d3.scaleRadial([-1, -2]).clamp(true)(-0.5), -1);
  assert.strictEqual(d3.scaleRadial().clamp(true)(-0.5), 0);
  assert.strictEqual(d3.scaleRadial([-0.25, 0], [1, 2]).clamp(true)(-0.5), 1);
});
